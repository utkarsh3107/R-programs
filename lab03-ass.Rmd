...
ggplot(by_year, aes(x=gdpPercap, y=lifeExp)) + geom_point() + scale_x_log10()
```
...
ggplot(by_year, aes(x=pop, y=lifeExp, color=year)) + geom_point() + scale_x_log10()
```
...
ggplot(gapminder, aes(pop)) + geom_density() + facet_wrap(~year) + scale_x_log10()
```
...
```{r A14}
by_continent <- gapminder
ggplot(by_continent, aes(x= lifeExp)) +   geom_histogram() 
```
...
ggplot(by_continent, aes(x= gdpPercap, y=lifeExp, color=continent)) + geom_point()+ facet_grid(~continent) + geom_smooth(method="loess") 
```
...
q2_a <- gapminder %>% filter(year == 1997) %>% filter(country %in% c("Ireland", "Brazil")) %>%  arrange(desc(lifeExp))
q2_a
q2_b <- gapminder %>% filter(year == 1997) %>% filter(country %in% c("Ireland", "Brazil")) %>%  arrange(desc(gdpPercap))
q2_b
```
...
a. Make a scatterplot as shown below with pop on the x-axis and lifeExp on the y-axis using gapminder_2007. Does there appear to be a relationship between population and life expectancy? One line of code is provided for creating `gapminder_2007 gapminder_2007 <- gapminder %>% filter(year == 2007)` Life expectancy was low for most of the countries other than 2.  
